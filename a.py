...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.start_canvas = tk.Canvas(self, width=2*w, height=40, bg='white')
        self.start_canvas.grid(row=1, column=0, columnspan=6)

        self.janken_btn = tk.Button(self, text='じゃんけん！',command=self.janken_start,width=50,height=2)
        self.janken_btn.grid(row=2,column=1, columnspan=4)

        # 結果表示領域の初期化
        self.result_canvas = tk.Canvas(self, width=2*w, height=30, bg='white')
        self.result_canvas.grid(row=3, column=0, columnspan=6)

        # クリアボタンの初期化
        self.clear_btn = tk.Button(self, text='集計のクリア', command=self.clear)
        self.clear_btn.grid(row=5, column=4, columnspan=2)
    ...
    def janken_start(self):
        ...
        self.start_canvas.create_text(600,20,text="最初はグー！",font = ('FixedSys', 20))
        sleep(2)
        self.start_canvas.delete('all')
        self.start_canvas.create_text(600,20,text="じゃんけん",font = ('FixedSys', 20))
        sleep(1)
        self.start_canvas.delete('all')
        self.start_canvas.create_text(600,20,text="ぽん！！",font = ('FixedSys', 20))
        sleep(0.5)
        human_hand = recognition.main(3)
        if human_hand == 0:
            self.human_gu()
        elif human_hand == 1:
            self.human_choki()
...
app.master.title('じゃんけんマシーン（強くなる）')
app.mainloop()
